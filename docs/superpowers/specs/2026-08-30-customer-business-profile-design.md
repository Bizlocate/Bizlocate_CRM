...
> **DEPLOY ORDERING:** this migration must be run against the production Supabase project BEFORE this branch is deployed. `lib/store.tsx`'s `addCustomer` unconditionally inserts all 15 new columns added by step 2 above — if the app code ships before the migration runs, `addCustomer` fails outright (`{ok: false}`) and customer creation breaks entirely, not just the new fields.
