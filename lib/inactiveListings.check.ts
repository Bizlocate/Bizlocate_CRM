...
import type { Activity, AssignmentEvent, Customer, RemovalRequest, Task } from "./types.ts";
...
function task(overrides: Partial<Task> & { id: string; customerId: string; userId: string }): Task {
  return { title: "T", due: "", done: false, ...overrides };
}
...
// --- computeSlotAges: a pending task for the slot's assignee suppresses pull/warn ---
{
  const c = customer({ id: "c4", assignedToUserId: "u1", pool1: "ACTIVE", createdAt: daysAgo(40) });
  const tasks: Task[] = [task({ id: "t1", customerId: "c4", userId: "u1", done: false })];
  const age = computeSlotAges([c], [], [], tasks, NOW)[0];
  assert.strictEqual(age.hasPendingTask, true);
  assert.strictEqual(isStalePastPull(age), false, "pending task blocks the auto-pull even past 30 days");
  assert.strictEqual(isWarnZone(age), false, "pending task blocks the warn zone too");

  // A done task doesn't count.
  const doneOnly = [task({ id: "t2", customerId: "c4", userId: "u1", done: true })];
  assert.strictEqual(computeSlotAges([c], [], [], doneOnly, NOW)[0].hasPendingTask, false);

  // A pending task belonging to a different user on the same customer doesn't count.
  const otherUsers = [task({ id: "t3", customerId: "c4", userId: "u9", done: false })];
  assert.strictEqual(computeSlotAges([c], [], [], otherUsers, NOW)[0].hasPendingTask, false);
}

// --- isStalePastPull / isWarnZone thresholds ---
{
  const activeJustUnder = { customerId: "x", slot: 1 as const, userId: "u", pool: "ACTIVE" as const, daysStale: ACTIVE_PULL_DAYS - 0.1, hasPendingTask: false };
  ...
  const potentialInWarn = { customerId: "x", slot: 1 as const, userId: "u", pool: "INACTIVE" as const, daysStale: POTENTIAL_WARN_DAYS + 1, hasPendingTask: false };
...
{
  const ages = [
    { customerId: "c1", slot: 1 as const, userId: "sales-a", pool: "ACTIVE" as const, daysStale: 26, hasPendingTask: false },
    { customerId: "c2", slot: 1 as const, userId: "sales-b", pool: "ACTIVE" as const, daysStale: 26, hasPendingTask: false },
    { customerId: "c3", slot: 1 as const, userId: "sales-c", pool: "ACTIVE" as const, daysStale: 26, hasPendingTask: false },
  ];
  const users = [
...
// --- warnZoneSlotsFor: composes age + warn zone + scope, minus pending removals ---
{
  ...
  // A pending task for this assignee on w1 drops just that slot, same as a PENDING removal request.
  const pendingTask = [task({ id: "t9", customerId: "w1", userId: "sales-a", done: false })];
  assert.deepStrictEqual(warnZoneSlotsFor([c1, c2], [], [], users, viewer, [], pendingTask, NOW).map((a) => a.customerId), ["w2"]);
}
